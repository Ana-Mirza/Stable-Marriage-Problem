...
(define (get-men mpref)
  (if (null? mpref)
      null
      (append (list (caar mpref)) (get-men (cdr mpref)))))
...
(define (tail-recursion wpref acc)
  (if (null? wpref)
      acc
      (tail-recursion (cdr wpref) (append acc (list (caar wpref))))))

(define (get-women wpref)
  (tail-recursion wpref null))
...
(define (member? y list)
  (and (not (null? list))
      (or (equal? y (car list)) (member? y (cdr list)))))

(define (preferable? pref-list x y)
  (and (not (null? pref-list))
      (or (and (equal? (car pref-list) x) (member? y pref-list)) (preferable? (cdr pref-list) x y))))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (and (not (equal? (car p1-list) p2))
      (or (and (preferable? p1-list (car p1-list) p2) (preferable? (get-pref-list pref2 (car p1-list)) p1 (get-partner engagements (car p1-list))))
