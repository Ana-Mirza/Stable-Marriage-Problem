...
(define (match person engagements pref1 pref2 queue)
  (let iter ([list1 (get-pref-list pref1 person)])
    (let* ([pref_pers (lambda () (car list1))] [get_partner (lambda(x) (get-partner x (pref_pers)))]
           [update (lambda(x) (update-engagements x (pref_pers) person))])
      (cond
        ((null? list1) (cons (cons #f person) engagements))
        ((and (not (false? (get_partner engagements)))
              (preferable? (get-pref-list pref2 (pref_pers)) person (get_partner engagements)))
         (match (get_partner engagements) (update engagements) pref1 pref2 queue))
        ((and (false? (get_partner engagements))
              (false? (member (pref_pers) queue))) (update engagements))
        (else (iter (cdr list1)))))))
...
(define (path-to-stability engagements mpref wpref queue)
  (let iter ([q queue] [eng engagements])
    (let ([reversed (lambda (y) (map (lambda (x) (cons (cdr x) (car x))) y))])
      (cond
        ((null? q) eng)
        ((not (member (car q) (get-women wpref))) (iter (cdr q) (match (car q) eng mpref wpref (cdr q))))
        (else (iter (cdr q) (reversed (match (car q) (reversed eng) wpref mpref (cdr q)))))))))
...
(define (update-stable-match engagements mpref wpref)
    (let* ([reversed (lambda(y) (map (lambda (x) (cons (cdr x) (car x))) y))]
           ...
           [room-engagements (filter (lambda (x) (nor (better-match-exists? (cdr x) (car x) (get-pref-list mpref (cdr x)) wpref engagements)
                          (better-match-exists? (car x) (cdr x) (get-pref-list wpref (car x)) mpref (reversed engagements)))) engagements)]
...
(define (stream-zip-with f k s2)
  (if (stream-empty? s2) s2
      (let ([k (f k (car (stream-first s2)) (cdr (stream-first s2)))])
        (stream-cons k (stream-zip-with f k (stream-rest s2))))))


(define (build-stable-matches-stream pref-stream)
  (if (stream-empty? pref-stream) pref-stream
      (stream-cons (gale-shapley (car (stream-first pref-stream)) (cdr (stream-first pref-stream)))
                   (stream-zip-with update-stable-match
                                    (gale-shapley (car (stream-first pref-stream)) (cdr (stream-first pref-stream)))
